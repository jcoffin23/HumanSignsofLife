...
tgtStruct.human = [1,1];
tgtStruct.offset = [10,20];
tgtStruct.yoffset= zeros(1,2);
tgtStruct.zoffset = zeros(1,2);
...
tgtinterSpace = 23:.1:25;
eVec = zeros(size(tgtinterSpace));

for tgtLoc = 1:length(tgtinterSpace)
tgtStruct.offset = [25,tgtinterSpace(tgtLoc)];
[frError,fhError,rangeRes,chestSig,peakFFT,tgtStructOut,recvPow] = singleMCTrial_bw(inputStruct,tgtStruct);
hact = tgtStructOut.fhActual(1);
pErr = abs(fhError)/hact;
pErrResp = frError/tgtStructOut.frActual(1);
eVec(tgtLoc) = pErrResp;
end

plot(25-tgtinterSpace,eVec)

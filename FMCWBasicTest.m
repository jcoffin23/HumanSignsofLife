...
L = 2^nextpow2(length(xr(:,1)));
xbo = 1:Nsweep;

freq_res = fs/L;
freqGrid = (0:L/2+1).'*freq_res;

rangeVec = beat2range(freqGrid,sweep_slope);

rngDat = fft(xr,L,1);

rngDat = rngDat(1:L/2+1,:)
% rngDat = fftshift(rngDat,1);
figure
imagesc(xbo,rangeVec,abs(rngDat))
title('Range vs Sweep number')
xlabel('Sweep Number')

...
numSamps = 25000; %this is known because of internal decisions. bad code?
...
tgtStruct.legacyHR = [0]; %Set to 0 to generate HRV data

lenTest = 100000;
huPhase = zeros(numSamps,lenTest);
heartActual= zeros(numSamps,lenTest);
clutPhase = zeros(numSamps,lenTest);
respActual = zeros(lenTest,1);
for k = 1:lenTest
    
tgtStruct.offset = 5 + (25-5).*rand(1,1);
...
respActual(k) = tgtStructOut.frActual;
...
save('mlDat_fs500_noHRV.mat','huPhase','heartActual','respActual','-v7.3')
save('mlDat_fs500_noHRV_raw.mat','-v7.3')

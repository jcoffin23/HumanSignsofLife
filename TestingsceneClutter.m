...
stvVec = -40:1:0;
eVec = zeros(5,length(stvVec));
for avg = 1:5
    for k = 1:length(stvVec)
        
        % tgtStruct.offset = [10];
        inputStruct.binError = 0;
        inputStruct.stv = stvVec(k);
...
meVec = mean(eVec,1);
plot(stvVec,meVec)
title('Resp Error vs Scene Clutter Power')
xlabel('Clutter Power')
ylabel('Percent respiration Error')
% save('sceneClutTest.mat')

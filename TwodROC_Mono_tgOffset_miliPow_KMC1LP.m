...
powVec = linspace(.01,80,20); %18dbm
...
tgtStruct.legacyHR=0; %Set to use constant heart rate
tgtStruct.human = [1];
tgtStruct.offset = [20];
tgtStruct.yoffset= [0];
tgtStruct.zoffset = [0];
...
%% For Loop
peakFFTMat = zeros(numPow,numVar);
...
heartActualMat=zeros([size(peakFFTMat), 10000]); %Must be size of ROC arrays and number of phase samples (fs = 500 * 20 seconds = 10k samples)
...
%     HeartErrVec = zeros(1,numMCTrials);
    ...
    for trialNum = 1:numPow
        ...
        inputStruct.gain =  40;
        inputStruct.miliPow = powVec(trialNum);
...
imagesc(offsetVec,powVec,respMat)

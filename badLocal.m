...
tgtStruct.human = [1];
tgtStruct.offset = [10];
tgtStruct.yoffset= zeros(1,1);
tgtStruct.zoffset = zeros(1,1);
...
inputStruct.stv = 25;
binEV = 0:2:20;
eVec = zeros(size(binEV));

for k = 1:length(binEV)
    
% tgtStruct.offset = [10];
inputStruct.binError = binEV(k);
[frError,fhError,rangeRes,chestSig,peakFFT,tgtStructOut,recvPow] = singleMCTrial_localTe(inputStruct,tgtStruct);
...
plot(binEV*bw2rangeres(bw),eVec)
title('Resp error vs incorrect target range')
xlabel('Range Error (m)')
ylabel('Percent respiration Error')
bw2rangeres(bw)

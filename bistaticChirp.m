% BISTATICCHIRP simulation of a single FMCW Chrip for a bistatic radar
% setup
%
% Syntax:
%  [rxSig] = bistaticChirp(transmitter,receiver,channel,radiator,collector,beamformer,tgt,txMotion,rxMotion,dt)
% 
% Inputs:
%
%  Required:
...
function [rxSig,tgtPlat,rxPlat,txPlat] = bistaticChirp(targets,transmitter,receiver,txchannel,rxchannel,radiator,collector,beamformer,tgtPlat,rxPlat,txPlat,dt,waveform,numTgt)




%Update Rx and TX movement (likly unmoving, but could move in future)
[txPos,txVel,txAx] = txPlat(dt); 
[rxPos,rxVel,rxAx] = rxPlat(dt);

%Update target movement for EACH target
[tgtPos,tgtVel,tgtAx] = tgtPlat(dt);
refSig = waveform();
txSig = transmitter(refSig); %Simulate propagation of puse 

[tgtRng,tgtAng]=rangeangle(tgtPos,txPos,txAx); %Get angle as seen by transmiter

rxSig = radiator(txSig,tgtAng,txAx); %Radiate the targets

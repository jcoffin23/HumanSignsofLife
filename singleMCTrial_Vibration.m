function [frError,fhError,rangeRes,chestSig,peakFFT,tgtStructOut,recvPow] = singleMCTrial_Vibration(inStruct,tgtStruct)
%% Constants
fc = inStruct.fc;
bw = inStruct.bw;
...
% [fftOut,f] = jct.freqResponse.easyFFT(vibrationY,fsChest,10000)
% 
% plot(f,abs(fftOut))
...
if inStruct.enableVibration
    
    oMat = azelaxes(0,0);
    
    onesVec = ones(size(t'));
    % vibrationX = zeros(size(t'));
    
    vibrationY = zeros(size(t'));
    
    vibrationX = vibrationY;
    
    for k = 1:size(inStruct.vibration,2)
        amp = inStruct.vibration(2,k);
        fvib = inStruct.vibration(1,k);
        vibrationY = vibrationY + amp*sin(2*pi*fvib*t)';
    end

    
    wpsRX = [t';vibrationX; -rxRad+ vibrationY; 0*onesVec]';
    wpsTX = [t';vibrationX; rxRad+ vibrationY; 0*onesVec]';
    rxPlatform = phased.Platform('MotionModel','Custom','CustomTrajectory',wpsRX,'InitialOrientationAxes',oMat,'OrientationAxesOutputPort',true);
    txPlatform = phased.Platform('MotionModel','Custom','CustomTrajectory',wpsTX,'InitialOrientationAxes',oMat,'OrientationAxesOutputPort',true);
else
...
if calcRangePlot
    ...
    rngDat = fft(reflectedSigTemp,size(reflectedSigTemp,1),1); %Calculated beat Frequencies using FFT
    
    
    
    q = 5000
    rangeVec=rangeVec(1:q);
    rngDat = rngDat(1:q,:); %Take only positive Freqencies
    ...
    if showDop
        ...
        hamWin = hamming(size(toDop,2));
        dopResp = fftshift(fft(toDop.*hamWin',N,2),2);
